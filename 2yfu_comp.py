...
filename = '~/resilience/2yfu_complete.csv'
...
def predict_collect_save(data_pred, data_collect, y_true, test_index,
    ...
    return
for split, (train_index, test_index) in enumerate(cv.split(X, y.to_numpy().flatten(), groups.to_numpy().flatten())):
    scores = {}
...
scores.to_csv('~/resilience/2yfu_results_complete.csv')
full = np.array(data,dtype=object)
np.savetxt('pred_complete.csv',full.reshape(10,-1),delimiter=',',fmt='%s')
