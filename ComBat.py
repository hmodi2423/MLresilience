...
filename = '~/resilience/neuraltest.csv'
abcd = pd.read_csv(filename)

new_abcd = abcd.dropna()

new_abcd.columns = new_abcd.columns.astype(str)
X = new_abcd.drop(columns=['src_subject_id','site_id_l','nihtbx_fluidcomp_uncorrected', 'cbcl_scr_syn_totprob_r'], axis=1)
...
y = pd.DataFrame(new_abcd, columns=['nihtbx_fluidcomp_uncorrected'])
...
scores.to_csv('~/resilience/250326_neuralcogresults.csv')
full = pd.concat(data)
full.to_csv('~/resilience/neural_cogpred.csv')
